...
train_pct = 0.8#0.8#0.4#0.1
valid_pct = 0.1#0.1#0.2#0.1
test_pct = 0.1#0.1#0.4#0.8
...
# Run K times.
    ...
    for i in range(0,K):
        ...
        modelNB3.add(layers.Conv2D(64, (3, 3), activation='relu', input_shape=(28, 28,1)))
        modelNB3.add(layers.MaxPooling2D((2, 2)))
        modelNB3.add(layers.Conv2D(32, (3, 3), activation='relu', input_shape=(28, 28,1)))
        modelNB3.add(layers.MaxPooling2D((2, 2)))
        ...
        model.compile(optimizer=opt,
                ...
                metrics=['accuracy'])
        history=None
        history = model.fit(train_images, train_labels, epochs=120, validation_split=test_pct/(train_pct+valid_pct),batch_size=32)
        ...
        plt.savefig(f'tensorflow/figs/relu/train/test-{figname}-{now}.png')
...
def optimize_learning_rate():
    #learning_rate_range = np.logspace(-6,-4,10)
    ...
    now =arrow.now().format('HH-mm-ss')
    plt.legend(loc='lower right')
    plt.savefig(f'tensorflow/figs/relu/optimization/learning-rate-{figname}-{now}.png')


def run_optimized():
    K=3
    train_acc_mean,test_acc_mean = run_K_times(K,train_images,test_images,train_labels,test_labels,6.5e-6)
