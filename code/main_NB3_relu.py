...
train_pct = 0.4#0.8#0.4#0.1
valid_pct = 0.2#0.1#0.2#0.1
test_pct = 0.4#0.1#0.4#0.8
...
# Run K times.
    ...
    for i in range(0,K):
        ...
        model.compile(optimizer=opt,
                ...
                metrics=['accuracy'])
        history=None
        history = model.fit(train_images, train_labels, epochs=80, validation_split=valid_pct/(train_pct+valid_pct),batch_size=32)
...
def optimize_learning_rate():
    learning_rate_range = np.logspace(-6,-1,10)
...
def run_optimized():
    K=3
    train_acc_mean,test_acc_mean = run_K_times(K,train_images,test_images,train_labels,test_labels,2e-4)
